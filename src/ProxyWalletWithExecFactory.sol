...
import "./ProxyWalletWithExec.sol";

contract ProxyWalletWithExecFactory is Ownable {
    ...
    function deploy(bytes32 _salt) external returns (address wallet) {
        bytes memory bytecode = abi.encodePacked(type(ProxyWalletWithExec).creationCode, abi.encode(msg.sender));
    ...
    function computeAddress(bytes32 _salt, address _owner) external view returns (address) {
        bytes memory bytecode = abi.encodePacked(type(ProxyWalletWithExec).creationCode, abi.encode(_owner));
